# NeetCode 150 Coding Interview Preparation

## 🚀 Overview

This repository documents my journey through the NeetCode 150 problem set, designed to comprehensively prepare for technical interviews at top tech companies. Each problem is solved with multiple approaches, focusing on optimal time and space complexity.

## 🌟 Why This Repository?

- 🏆 **Comprehensive Coverage**: Includes all **150 problems** categorized by topic.
- 📂 **Well-Organized**: Each problem is classified under relevant categories for easy navigation.
- 🚀 **Practical Solutions**: Solutions written in **C**, **C++**,and **Python**, and others.
- 📈 **Progress Tracking**: Keep tabs on solved problems, My [SpreedSheet](https://docs.google.com/spreadsheets/d/1vGJ-WOSzLkEFqGqLr-96vAz7uYljKoX_nmF67LD29TU/edit?usp=sharing), (you can copy).
- 🎯 **Targeted Practice**: From **basic concepts** to **advanced problems**.

----

## 📊 Progress Tracker

- [ ] ✅ 🧮 **Arrays & Hashing** (0/9)
- [ ] ❎ 🔄 **Two Pointers** (0/5)  
- [ ] ❎ 📚 **Stack** (0/7)  
- [ ] ❎ 🚪 **Sliding Window** (0/6)  
- [ ] ❎ 🔍 **Binary Search** (0/7)  
- [ ] ❎ 🔗 **Linked List** (0/11)  
- [ ] ❎ 🌳 **Trees** (0/15)  
- [ ] ❎ 🧵 **Tries** (0/3)  
- [ ] ❎ 🔑 **Heap / Priority Queue** (0/7)  
- [ ] ❎ 🔑 **Backtracking** (0/9)  
- [ ] ❎ 🗺️ **Graphs** (0/13)  
- [ ] ❎ 🌐 **Advanced Graphs** (0/6)  
- [ ] ❎ 🏃 **1D Dynamic Programming** (0/12)  
- [ ] ❎ 🏗️ **2D Dynamic Programming** (0/11)  
- [ ] ❎ 🏆 **Greedy** (0/8)  
- [ ] ❎ 🕒 **Intervals** (0/6)  
- [ ] ❎ 📐 **Math & Geometry** (0/8)  
- [ ] ❎ 💡 **Bit Manipulation** (0/7)  

---

## 🧠 Learning Approach

1. **Problem Understanding**: Deeply analyze problem constraints
2. **Multiple Solutions**: Implement brute force, optimized, and most optimal solutions
3. **Complexity Analysis**: Detailed time and space complexity for each approach
4. **Explanatory Comments**: Thorough code commenting explaining logic

## 🛠 Technologies & Tools

![C](https://img.shields.io/badge/C-A8B9CC?style=flat&logo=C&logoColor=white)

![C++](https://img.shields.io/badge/C%2B%2B-00599C?style=flat&logo=c%2B%2B&logoColor=white)

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)

![LeetCode](https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=LeetCode&logoColor=black)

![Data Structures](https://img.shields.io/badge/Data%20Structures-Algorithms-blueviolet?style=for-the-badge)


## 💡 Solving Strategy

- **Time Complexity**: O(n) or better
- **Space Complexity**: O(1) when possible
- **Edge Case Handling**: Comprehensive testing
- **Code Readability**: Clean, modular solutions

## 🔍 How to Use This Repository


1. **Clone this repository**:
   ```bash
   git clone https://github.com/yomazini/NeetCode-150-Solutions.git
   ```
2. **Navigate to the problem category**:
   ```bash
   cd Arrays_and_Hashing
   ```
3. **Run the solution in your preferred programming language**:
   ```bash
   # if Python
   python3 solution.py
   ```
   ```bash
   # if C run it with gcc
	gcc main.c 
   ```
	```bash
	# if C++ run it with g
	g++ main.cpp
	```


## 📚 Resources

- [NeetCode Roadmap](https://neetcode.io/roadmap)
- [LeetCode](https://leetcode.com)

## 📈 Personal Growth

Tracking my learning journey, algorithmic thinking, and problem-solving skills through systematic practice.

---

## 📚 Categories & Problems

Below is a categorized list of all the problems. Click on a problem to visit it on LeetCode.

### 📂 Arrays & Hashing
| Problem                                                                 | Link                                                                             |
|-------------------------------------------------------------------------|----------------------------------------------------------------------------------|
| Contains Duplicate                                                      | [Link](https://leetcode.com/problems/contains-duplicate/)                       |
| Valid Anagram                                                           | [Link](https://leetcode.com/problems/valid-anagram/)                            |
| Two Sum                                                                 | [Link](https://leetcode.com/problems/two-sum/)                                  |
| Group Anagrams                                                          | [Link](https://leetcode.com/problems/group-anagrams/)                           |
| Top K Frequent Elements                                                 | [Link](https://leetcode.com/problems/top-k-frequent-elements/)                  |
| Product of Array Except Self                                            | [Link](https://leetcode.com/problems/product-of-array-except-self/)             |
| Valid Sudoku                                                            | [Link](https://leetcode.com/problems/valid-sudoku/)                             |
| Encode and Decode Strings                                               | [Link](https://leetcode.com/accounts/login/?next=/problems/encode-and-decode-strings/) |
| Longest Consecutive Sequence                                            | [Link](https://leetcode.com/problems/longest-consecutive-sequence/)             |

### 📂 Two Pointers
| Problem                                                                 | Link                                                                             |
|-------------------------------------------------------------------------|----------------------------------------------------------------------------------|
| Valid Palindrome                                                        | [Link](https://leetcode.com/problems/valid-palindrome/)                         |
| Two Sum II                                                              | [Link](https://leetcode.com/problems/two-sum-ii-input-array-is-sorted/)         |
| 3Sum                                                                    | [Link](https://leetcode.com/problems/3sum/)                                     |
| Container with Most Water                                               | [Link](https://leetcode.com/problems/container-with-most-water/)                |
| Trapping Rain Water                                                     | [Link](https://leetcode.com/problems/trapping-rain-water/)                      |

### 📂 Sliding Window
| Problem                                                                 | Link                                                                             |
|-------------------------------------------------------------------------|----------------------------------------------------------------------------------|
| Best Time to Buy & Sell Stock                                           | [Link](https://leetcode.com/problems/best-time-to-buy-and-sell-stock/)          |
| Longest Substring Without Repeating Characters                         | [Link](https://leetcode.com/problems/longest-substring-without-repeating-characters/) |
| Longest Repeating Character Replacement                                 | [Link](https://leetcode.com/problems/longest-repeating-character-replacement/)  |
| Permutation in String                                                   | [Link](https://leetcode.com/problems/permutation-in-string/)                    |
| Minimum Window Substring                                                | [Link](https://leetcode.com/problems/minimum-window-substring/)                 |

### 📂 Stack
| Problem                                                                 | Link                                                                             |
|-------------------------------------------------------------------------|----------------------------------------------------------------------------------|
| Valid Parentheses                                                       | [Link](https://leetcode.com/problems/valid-parentheses/)                        |
| Min Stack                                                               | [Link](https://leetcode.com/problems/min-stack/)                                |
| Evaluate Reverse Polish Notation                                        | [Link](https://leetcode.com/problems/evaluate-reverse-polish-notation/)         |
| Generate Parentheses                                                    | [Link](https://leetcode.com/problems/generate-parentheses/)                     |
| Daily Temperatures                                                      | [Link](https://leetcode.com/problems/daily-temperatures/)                       |
| Car Fleet                                                               | [Link](https://leetcode.com/problems/car-fleet/)                                |
| Largest Rectangle in Histogram                                          | [Link](https://leetcode.com/**problems**/largest-rectangle-in-histogram/)           |


### 📂 Binary Search
| Problem                               | Link                                                                 |
|---------------------------------------|----------------------------------------------------------------------|
| Binary Search                         | [Link](https://leetcode.com/problems/binary-search/)                |
| Search a 2D Matrix                    | [Link](https://leetcode.com/problems/search-a-2d-matrix/)           |
| Koko Eating Bananas                   | [Link](https://leetcode.com/problems/koko-eating-bananas/)          |
| Search Rotated Sorted Array           | [Link](https://leetcode.com/problems/search-in-rotated-sorted-array/) |
| Find Minimum in Rotated Sorted Array  | [Link](https://leetcode.com/problems/find-minimum-in-rotated-sorted-array/) |
| Time Based Key-Value Store            | [Link](https://leetcode.com/problems/time-based-key-value-store/)   |
| Find Median of Two Sorted Arrays      | [Link](https://leetcode.com/problems/median-of-two-sorted-arrays/)  |

### 📂 Linked List
| Problem                               | Link                                                                 |
|---------------------------------------|----------------------------------------------------------------------|
| Reverse Linked List                   | [Link](https://leetcode.com/problems/reverse-linked-list/)          |
| Merge Two Linked Lists                | [Link](https://leetcode.com/problems/merge-two-sorted-lists/)       |
| Reorder List                          | [Link](https://leetcode.com/problems/reorder-list/)                 |
| Remove Nth Node from End of List      | [Link](https://leetcode.com/problems/remove-nth-node-from-end-of-list/) |
| Copy List with Random Pointer         | [Link](https://leetcode.com/problems/copy-list-with-random-pointer/) |
| Add Two Numbers                       | [Link](https://leetcode.com/problems/add-two-numbers/)              |
| Linked List Cycle                     | [Link](https://leetcode.com/problems/linked-list-cycle/)            |
| Find the Duplicate Number             | [Link](https://leetcode.com/problems/find-the-duplicate-number/)    |
| LRU Cache                             | [Link](https://leetcode.com/problems/lru-cache/)                    |
| Merge K Sorted Lists                  | [Link](https://leetcode.com/problems/merge-k-sorted-lists/)         |
| Reverse Nodes in K-Group              | [Link](https://leetcode.com/problems/reverse-nodes-in-k-group/)     |


### 📂 Trees
| Problem                               | Link                                                                 |
|---------------------------------------|----------------------------------------------------------------------|
| Invert Binary Tree                    | [Link](https://leetcode.com/problems/invert-binary-tree/)           |
| Maximum Depth of Binary Tree          | [Link](https://leetcode.com/problems/maximum-depth-of-binary-tree/) |
| Diameter of a Binary Tree             | [Link](https://leetcode.com/problems/diameter-of-binary-tree/)      |
| Balanced Binary Tree                  | [Link](https://leetcode.com/problems/balanced-binary-tree/)         |
| Same Tree                             | [Link](https://leetcode.com/problems/same-tree/)                    |
| Subtree of Another Tree               | [Link](https://leetcode.com/problems/subtree-of-another-tree/)      |
| Lowest Common Ancestor of a BST       | [Link](https://leetcode.com/problems/lowest-common-ancestor-of-a-binary-search-tree/) |
| Binary Tree Level Order Traversal     | [Link](https://leetcode.com/problems/binary-tree-level-order-traversal/) |
| Binary Tree Right Side View           | [Link](https://leetcode.com/problems/binary-tree-right-side-view/)  |
| Count Good Nodes in a Binary Tree     | [Link](https://leetcode.com/problems/count-good-nodes-in-binary-tree/) |
| Validate Binary Search Tree           | [Link](https://leetcode.com/problems/validate-binary-search-tree/)  |
| Kth Smallest Element in a BST         | [Link](https://leetcode.com/problems/kth-smallest-element-in-a-bst/) |
| Construct Tree from Preorder and Inorder Traversal | [Link](https://leetcode.com/problems/construct-binary-tree-from-preorder-and-inorder-traversal/) |
| Binary Tree Max Path Sum              | [Link](https://leetcode.com/problems/binary-tree-maximum-path-sum/) |
| Serialize and Deserialize Binary Tree | [Link](https://leetcode.com/problems/serialize-and-deserialize-binary-tree/) |

### 📂 Tries
| Problem                               | Link                                                                 |
|---------------------------------------|----------------------------------------------------------------------|
| Implement Trie                        | [Link](https://leetcode.com/problems/implement-trie-prefix-tree/)   |
| Design Add and Search Word Data Structure | [Link](https://leetcode.com/problems/design-add-and-search-words-data-structure/) |
| Word Search II                        | [Link](https://leetcode.com/problems/word-search-ii/)               |


### 📂 Heap / Priority Queue
| Problem                               | Link                                                                 |
|---------------------------------------|----------------------------------------------------------------------|
| Kth Largest Element in a Stream       | [Link](https://leetcode.com/problems/kth-largest-element-in-a-stream/) |
| Last Stone Weight                     | [Link](https://leetcode.com/problems/last-stone-weight/)            |
| K Closest Points to Origin            | [Link](https://leetcode.com/problems/k-closest-points-to-origin/)   |
| Kth Largest Element in an Array       | [Link](https://leetcode.com/problems/kth-largest-element-in-an-array/) |
| Task Scheduler                        | [Link](https://leetcode.com/problems/task-scheduler/)               |
| Design Twitter                        | [Link](https://leetcode.com/problems/design-twitter/)               |
| Find Median from Data Stream          | [Link](https://leetcode.com/problems/find-median-from-data-stream/) |


### 📂 Backtracking
| Problem                               | Link                                                                 |
|---------------------------------------|----------------------------------------------------------------------|
| Subsets                               | [Link](https://leetcode.com/problems/subsets/)                      |
| Combination Sum                       | [Link](https://leetcode.com/problems/combination-sum/)              |
| Permutations                          | [Link](https://leetcode.com/problems/permutations/)                 |
| Subsets II                            | [Link](https://leetcode.com/problems/subsets-ii/)                   |
| Combination Sum II                    | [Link](https://leetcode.com/problems/combination-sum-ii/)           |
| Word Search                           | [Link](https://leetcode.com/problems/word-search/)                  |
| Palindrome Partitioning               | [Link](https://leetcode.com/problems/palindrome-partitioning/)      |
| Letter Combinations of a Phone Number | [Link](https://leetcode.com/problems/letter-combinations-of-a-phone-number/) |
| N-Queens                              | [Link](https://leetcode.com/problems/n-queens/)                     |

### 📂 Graphs
| Problem                               | Link                                                                 |
|---------------------------------------|----------------------------------------------------------------------|
| Number of Islands                     | [Link](https://leetcode.com/problems/number-of-islands/)            |
| Clone Graph                           | [Link](https://leetcode.com/problems/clone-graph/)                  |
| Max Area of Island                    | [Link](https://leetcode.com/problems/max-area-of-island/)           |
| Pacific Atlantic Waterflow            | [Link](https://leetcode.com/problems/pacific-atlantic-water-flow/)  |
| Surrounded Regions                    | [Link](https://leetcode.com/problems/surrounded-regions/)           |
| Rotting Oranges                       | [Link](https://leetcode.com/problems/rotting-oranges/)              |
| Walls and Gates                       | [Link](https://leetcode.com/problems/walls-and-gates/)              |
| Course Schedule                       | [Link](https://leetcode.com/problems/course-schedule/)              |
| Course Schedule II                    | [Link](https://leetcode.com/problems/course-schedule-ii/)           |
| Redundant Connection                  | [Link](https://leetcode.com/problems/redundant-connection/)         |
| Number of Connected Components in Graph | [Link](https://leetcode.com/problems/number-of-connected-components-in-an-undirected-graph/) |
| Graph Valid Tree                      | [Link](https://leetcode.com/problems/graph-valid-tree/)             |
| Word Ladder                           | [Link](https://leetcode.com/problems/word-ladder/)                  |

---

Let me know if you'd like to continue or expand further!

---

Here is the continuation of the table, categorized as per your request:


### 📂 Advanced Graphs
| Problem                                   | Link                                                                 |
|-------------------------------------------|----------------------------------------------------------------------|
| Reconstruct Itinerary                     | [Link](https://leetcode.com/problems/reconstruct-itinerary/)        |
| Min Cost to Connect all Points            | [Link](https://leetcode.com/problems/min-cost-to-connect-all-points/) |
| Network Delay Time                        | [Link](https://leetcode.com/problems/network-delay-time/)            |
| Swim in Rising Water                      | [Link](https://leetcode.com/problems/swim-in-rising-water/)          |
| Alien Dictionary                          | [Link](https://leetcode.com/problems/alien-dictionary/)              |
| Cheapest Flights with K Stops             | [Link](https://leetcode.com/problems/cheapest-flights-within-k-stops/) |


### 📂 1-D Dynamic Programming
| Problem                                   | Link                                                                 |
|-------------------------------------------|----------------------------------------------------------------------|
| Climbing Stairs                           | [Link](https://leetcode.com/problems/climbing-stairs/)               |
| Min Cost Climbing Stairs                  | [Link](https://leetcode.com/problems/min-cost-climbing-stairs/)      |
| House Robber                              | [Link](https://leetcode.com/problems/house-robber/)                  |
| House Robber II                           | [Link](https://leetcode.com/problems/house-robber-ii/)               |
| Longest Palindromic Substring             | [Link](https://leetcode.com/problems/longest-palindromic-substring/) |
| Palindrome Substrings                     | [Link](https://leetcode.com/problems/palindromic-substrings/)        |
| Decode Ways                               | [Link](https://leetcode.com/problems/decode-ways/)                   |
| Coin Change                               | [Link](https://leetcode.com/problems/coin-change/)                   |
| Maximum Product Subarray                  | [Link](https://leetcode.com/problems/maximum-product-subarray/)      |
| Word Break                                | [Link](https://leetcode.com/problems/word-break/)                    |
| Longest Increasing Subsequence            | [Link](https://leetcode.com/problems/longest-increasing-subsequence/) |
| Partition Equal Subset Sum                | [Link](https://leetcode.com/problems/partition-equal-subset-sum/)    |


### 📂 2-D Dynamic Programming
| Problem                                   | Link                                                                 |
|-------------------------------------------|----------------------------------------------------------------------|
| Unique Paths                              | [Link](https://leetcode.com/problems/unique-paths/)                  |
| Longest Common Subsequence                | [Link](https://leetcode.com/problems/longest-common-subsequence/)    |
| Best Time to Buy/Sell Stock With Cooldown | [Link](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-with-cooldown/) |
| Coin Change II                            | [Link](https://leetcode.com/problems/coin-change-2/)                 |
| Target Sum                                | [Link](https://leetcode.com/problems/target-sum/)                    |
| Interleaving String                       | [Link](https://leetcode.com/problems/interleaving-string/)           |
| Longest Increasing Path in a Matrix       | [Link](https://leetcode.com/problems/longest-increasing-path-in-a-matrix/) |
| Distinct Subsequences                     | [Link](https://leetcode.com/problems/distinct-subsequences/)         |
| Edit Distance                             | [Link](https://leetcode.com/problems/edit-distance/)                 |
| Burst Balloons                            | [Link](https://leetcode.com/problems/burst-balloons/)                |
| Regular Expression Matching               | [Link](https://leetcode.com/problems/regular-expression-matching/)   |


### 📂 Greedy
| Problem                                   | Link                                                                 |
|-------------------------------------------|----------------------------------------------------------------------|
| Maximum Subarray                          | [Link](https://leetcode.com/problems/maximum-subarray/)              |
| Jump Game                                 | [Link](https://leetcode.com/problems/jump-game/)                     |
| Jump Game II                              | [Link](https://leetcode.com/problems/jump-game-ii/)                  |
| Gas Station                               | [Link](https://leetcode.com/problems/gas-station/)                   |
| Hand of Straights                         | [Link](https://leetcode.com/problems/hand-of-straights/)             |
| Merge Triplets to Form Target Triplet     | [Link](https://leetcode.com/problems/merge-triplets-to-form-target-triplet/) |
| Partition Labels                          | [Link](https://leetcode.com/problems/partition-labels/)              |
| Valid Parenthesis String                  | [Link](https://leetcode.com/problems/valid-parenthesis-string/)      |

### 📂 Intervals
| Problem                                   | Link                                                                 |
|-------------------------------------------|----------------------------------------------------------------------|
| Insert Interval                           | [Link](https://leetcode.com/problems/insert-interval/)               |
| Merge Intervals                           | [Link](https://leetcode.com/problems/merge-intervals/)               |
| Non-Overlapping Intervals                 | [Link](https://leetcode.com/problems/non-overlapping-intervals/)     |
| Meeting Rooms                             | [Link](https://leetcode.com/problems/meeting-rooms/)                 |
| Meeting Rooms II                          | [Link](https://leetcode.com/problems/meeting-rooms-ii/)              |
| Minimum Interval to Include Each Query    | [Link](https://leetcode.com/problems/minimum-interval-to-include-each-query/) |


### 📂 Math & Geometry
| Problem                                   | Link                                                                 |
|-------------------------------------------|----------------------------------------------------------------------|
| Rotate Image                              | [Link](https://leetcode.com/problems/rotate-image/)                  |
| Spiral Matrix                             | [Link](https://leetcode.com/problems/spiral-matrix/)                 |
| Set Matrix Zeroes                         | [Link](https://leetcode.com/problems/set-matrix-zeroes/)             |
| Happy Number                              | [Link](https://leetcode.com/problems/happy-number/)                  |
| Plus One                                  | [Link](https://leetcode.com/problems/plus-one/)                      |
| Pow(x, n)                                 | [Link](https://leetcode.com/problems/powx-n/)                        |
| Multiply Strings                          | [Link](https://leetcode.com/problems/multiply-strings/)              |
| Detect Squares                            | [Link](https://leetcode.com/problems/detect-squares/)                |

### 📂 Bit Manipulation
| Problem                                   | Link                                                                 |
|-------------------------------------------|----------------------------------------------------------------------|
| Single Number                             | [Link](https://leetcode.com/problems/single-number/)                 |
| Number of 1 Bits                          | [Link](https://leetcode.com/problems/number-of-1-bits/)              |
| Counting Bits                             | [Link](https://leetcode.com/problems/counting-bits/)                 |
| Reverse Bits                              | [Link](https://leetcode.com/problems/reverse-bits/)                  |
| Missing Number                            | [Link](https://leetcode.com/problems/missing-number/)                |
| Sum of Two Integers                       | [Link](https://leetcode.com/problems/sum-of-two-integers/)           |
| Reverse Integer                           | [Link](https://leetcode.com/problems/reverse-integer/)               |

---

**Disclaimer**: Solutions are for learning purposes. Always strive to understand the underlying concepts!

---

## 🎭 Author

Made with ☕️ and ☕️☕️☕️ by Youssef Mazini (ymazini)
- 42 Intra: [ymazini](https://profile.intra.42.fr/users/ymazini)
- GitHub: [yomazini](https://github.com/yomazini)

## 🤝 Contribution

Open to discussions, alternative solutions, and optimization suggestions. Feel free to open an issue or submit a pull request!
